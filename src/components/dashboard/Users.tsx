...
const Users = () => {
  ...
  const updateEvents = (teamName: string, name?: string) => {
    if (teamName === "delete") {
      name &&
        getUserByName(name).then((res) => {
          updateUser(res, "delete").then(() => {
            Promise.all([getAllTeamMembers(), getLoggedInId()]).then(
              ([teamMembers, loggedInId]) => {
                const teamMember = teamMembers.find(
                  (item: ITeamMember) => item.personId === res.id
                );

                teamMember &&
                  loggedInId &&
                  deleteTeamMember(teamMember.id, loggedInId).then(() =>
      ...
      name &&
        getUserByName(name)
          .then((res) => {
            return getAllTeamMembers().then((items) => {
              const filteredItems = items.filter(
                (item) => item.personId === res.id
              );
          ...
          .then((filteredItems) => {
            return getLoggedInId().then((loggedInUserId) => {
          ...
          .then(({ filteredItems, loggedInUserId }) => {
            return (
              loggedInUserId &&
              UpdateTeamMembers(filteredItems, loggedInUserId).then(() => {
                setRefreshData(!refreshData);
              })
            );
          });
  ...
  useEffect(() => {
    Promise.all([getAllUsers(), getAllTeamMembers()]).then(
      ([userData, teamData]) => {
        Promise.all(
          userData
              ...
              (user, index) =>
                user.user.isRegistered !== false && (
                  <tr
                    ...
                    key={index}
                    onClick={() => {
                      handleUserDetailsData(user.user);
                  ...
                  >
                    ...
                    <td className="rowTitle">
                      {user.team?.registeredDate &&
                        formatDate(user.team?.registeredDate)}
                    </td>
                    <td className="rowTitle">
                      <svg
                        width="36"
