  ...
  return (
      ...
      <div className="row">
          {teamMembers && teamMembers.map((item: any, index: any) => (
            <div className="cardStyling  col-sm-3 mb-3" key={index} id={index === teamMembers.length - 1 ? 'blurred' : ''}>
              <img
                src={image}
                width={64}
                height={64}
              ...
              {index === teamMembers.length - 1 && <label className='invitationPending'>Invitation Pending</label> }
            </div>
          ))}
